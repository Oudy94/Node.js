...
app.post('/weather', async(req, res) => {
    ...
    const endpoint = `https://api.openweathermap.org/data/2.5/weather?q=${cityName}&APPID=${API_KEY}&units=${temperatureUnit}`;

    const fetch = await fetchData(endpoint);

    if (fetch.status === 200){
        if(temperatureUnit === "standard"){
            res.render('index', { weatherText: `The temperature in ${cityName} is ${fetch.data.main.temp} K`});
        }
        else if(temperatureUnit === "metric"){
            res.render('index', { weatherText: `The temperature in ${cityName} is ${fetch.data.main.temp} °C`});
        }
        else if(temperatureUnit === "imperial"){
            res.render('index', { weatherText: `The temperature in ${cityName} is ${fetch.data.main.temp} °F`});
        }
    }
    else{
